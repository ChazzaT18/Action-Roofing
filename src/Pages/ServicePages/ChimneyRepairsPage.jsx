import React, { useEffect } from "react";
import { Link } from "react-router-dom";
import {
  ShieldCheck,
  ArrowRight,
  Phone,
  AlertTriangle,
  CheckCircle2,
  Hammer,
  Droplets,
  Wind,
  Camera,
  FileText,
  Search,
  History,
  Eye,
  Trash2,
} from "lucide-react";
import ServicesCTA from "./ServicesCTA";

const ChimneyRepairsPage = ({ setCurrentPage }) => {
  useEffect(() => {
    setCurrentPage("services");
    window.scrollTo(0, 0);
  }, [setCurrentPage]);

  return (
    <div className="bg-white">
      {/* 1. HERO SECTION */}
      <section className="relative w-full min-h-[85vh] flex items-center bg-[url('/images/Chimney-repairs.jpg')] bg-center bg-cover text-white">
        {/* Dark Overlay */}
        <div className="absolute inset-0 bg-black/50 z-0" />

        <div className="max-w-screen-2xl w-full mx-auto px-4 md:px-10 relative z-10 text-center">
          {/* Main Content Wrapper - Safe zone for fixed header on mobile */}
          <div className="py-12 sm:pt-20 lg:pt-10 flex flex-col justify-center items-center">
            
            {/* Top Badge */}
            <div className="inline-flex items-center gap-2 bg-[#F9D759] text-[#243453] px-4 py-2 rounded-full font-black uppercase tracking-widest text-xs 2xl:text-sm mb-6 shadow-lg">
              <ShieldCheck size={18} />
              Structural Maintenance & Safety
            </div>

            {/* Heading */}
            <h1 className="text-3xl md:text-3xl lg:text-4xl 2xl:text-5xl font-black mb-6 leading-tight uppercase tracking-tight">
              Chimney Repairs in <br />
              <span className="text-[#F9D759]">Brighton & Hove</span>
            </h1>

            {/* Subtext */}
            <p className="text-lg sm:text-xl text-slate-100 leading-relaxed mb-8 font-medium max-w-3xl mx-auto">
              Professional repairs carried out carefully to keep your roofline
              watertight, structurally sound, and protected from coastal storm
              damage.
            </p>

            {/* CTA Buttons */}
            <div className="flex flex-col items-center pt-2">
              <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
                <Link
                  to="/contact-us"
                  className="px-8 py-4 2xl:px-10 2xl:py-5 2xl:text-xl bg-[#F9D759] hover:bg-[#F7CB28] text-[#243453] font-bold rounded-xl shadow-xl transition-all flex items-center justify-center gap-2 group transform active:scale-95 sm:hover:scale-105"
                >
                  Request a Quote
                  <ArrowRight
                    size={20}
                    className="group-hover:translate-x-1 transition-transform"
                  />
                </Link>
                <a
                  href="tel:07768981913"
                  className="px-8 py-4 2xl:px-10 2xl:py-5 2xl:text-xl bg-[#243453] backdrop-blur-sm hover:bg-[#243453] text-white border border-slate-500 font-bold rounded-xl shadow-xl transition-all flex items-center justify-center gap-2 transform active:scale-95 sm:hover:scale-105"
                >
                  <Phone size={20} className="text-[#F9D759]" />
                  Call Now<span className="max-sm:hidden">: 07768 981913</span>
                </a>
              </div>
              
              <p className="mt-6 text-xs sm:text-sm 2xl:text-base text-slate-300 italic font-medium">
                Fully insured • Clear advice • Clean, tidy workmanship
              </p>
            </div>
          </div>
        </div>
      </section>

      {/* 2. THE SYSTEM NARRATIVE - Reverted to Original 2-Column Style */}
      <section className="py-24 bg-white">
        <div className="max-w-screen-2xl mx-auto px-4 md:px-10">
          <div className="flex flex-col lg:flex-row gap-16 items-start">
            {/* Left Column: Brother's Wording */}
            <div className="lg:w-3/5">
              <h2 className="text-2xl md:text-3xl 2xl:text-4xl font-black uppercase mb-8 leading-tight text-[#243453]">
                Chimneys Are a Common{" "}
                <span className="text-[#F9D759]">Source of Problems</span>
              </h2>
              <div className="space-y-6 text-slate-600 text-lg font-medium leading-relaxed">
                <p>
                  Chimneys are one of the most exposed parts of a roof. Constant
                  exposure to wind, rain, and temperature changes means they are
                  more likely to develop issues over time.
                </p>
                <p>
                  Because they sit above the roofline and involve multiple
                  materials coming together, even small defects can allow water
                  to enter the roof structure. When problems occur, they often
                  show as leaks inside the property, staining around the chimney
                  breast, or visible deterioration externally.
                </p>
                <p className="border-l-4 border-[#F9D759] pl-6 italic text-[#243453]">
                  "We approach chimney repairs with care, focusing on
                  identifying the root cause of the issue and resolving it
                  properly."
                </p>
              </div>
            </div>

            {/* Right Column: Visual Checklist */}
            <div className="lg:w-2/5 w-full">
              <div className="bg-slate-50 border-2 border-slate-100 p-8 rounded-[2.5rem] relative overflow-hidden">
                <h4 className="text-[#243453] font-black uppercase tracking-widest text-sm mb-8 border-b border-slate-200 pb-4 text-center">
                  Diagnostic Focus
                </h4>
                <ul className="space-y-6 relative z-10">
                  {[
                    {
                      title: "Root Cause ID",
                      desc: "Finding the actual source of ingress, not applying short-term fixes.",
                      icon: Search,
                    },
                    {
                      title: "Lead Flashing",
                      desc: "Checking Code 4/5 lead seals for cracks, fatigue, or lift.",
                      icon: Droplets,
                    },
                    {
                      title: "Structural Health",
                      desc: "Assessing stack stability and masonry erosion over time.",
                      icon: Hammer,
                    },
                    {
                      title: "Weatherproofing",
                      desc: "Ensuring a consistent and watertight finish to the entire stack.",
                      icon: ShieldCheck,
                    },
                  ].map((item, i) => (
                    <li key={i} className="flex items-start gap-4">
                      <div className="bg-white p-2 rounded-lg shadow-sm border border-slate-100">
                        <item.icon className="text-[#F9D759]" size={20} />
                      </div>
                      <div>
                        <p className="text-[#243453] font-black uppercase text-xs tracking-wide mb-1">
                          {item.title}
                        </p>
                        <p className="text-slate-500 text-xs font-bold leading-relaxed">
                          {item.desc}
                        </p>
                      </div>
                    </li>
                  ))}
                </ul>
              </div>
            </div>
          </div>
        </div>
      </section>

      {/* 3. SIGNS OF DETERIORATION - Clean Grid, No Image */}
      <section className="py-24 bg-[#243453] text-white">
        <div className="max-w-screen-2xl mx-auto px-4 md:px-10">
          <div className="text-center mb-16">
            <h2 className="text-3xl lg:text-4xl font-black uppercase mb-4 leading-tight">
              Signs Your Chimney{" "}
              <span className="text-[#F9D759]">Needs Attention</span>
            </h2>
            <p className="text-slate-300 text-lg max-w-3xl mx-auto font-medium">
              Chimney issues don’t always appear suddenly. Addressing small
              defects early helps prevent more extensive and costly structural
              repairs later on.
            </p>
          </div>

          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              {
                t: "Damp Patches",
                d: "Staining on ceilings or internal walls near the chimney breast.",
                i: Droplets,
              },
              {
                t: "Mortar Erosion",
                d: "Cracking and visible breakdown in the mortar joints.",
                i: Hammer,
              },
              {
                t: "Flashing Wear",
                d: "Lead flashing that appears worn, lifted, or poorly fitted.",
                i: Eye,
              },
              {
                t: "Masonry Decay",
                d: "Visible deterioration or bricks beginning to crumble.",
                i: History,
              },
            ].map((item, index) => (
              <div
                key={index}
                className="p-8 bg-white/5 border border-white/10 rounded-3xl"
              >
                <item.i
                  className="text-[#F9D759] mb-4 transition-transform"
                  size={32}
                />
                <h4 className="text-lg font-black uppercase mb-2 tracking-wide">
                  {item.t}
                </h4>
                <p className="text-slate-400 text-sm leading-relaxed font-bold">
                  {item.d}
                </p>
              </div>
            ))}
          </div>

          <div className="mt-12 flex justify-center">
            <div className="inline-flex items-center gap-3 px-6 py-3 bg-white/5 rounded-full border border-white/10 text-slate-300 text-sm italic">
              <AlertTriangle size={18} className="text-[#F9D759]" />
              Loose pots or fallen masonry require an immediate safety
              inspection.
            </div>
          </div>
        </div>
      </section>

      {/* 4. SPECIALIST SERVICES (Different Layout: Two Wide Boxes) */}
      <section className="py-24 bg-white">
        <div className="max-w-screen-2xl mx-auto px-4 md:px-10">
          <div className="grid lg:grid-cols-2 gap-8">
            <div className="group p-10 bg-[#243453] text-white rounded-[3rem] transition-all">
              <Hammer className="text-[#F9D759] mb-6" size={48} />
              <h3 className="text-2xl font-black uppercase mb-4">
                Chimney Repointing
              </h3>
              <p className="text-slate-300 text-lg leading-relaxed mb-6 font-medium">
                Over time, mortar between chimney bricks breaks down due to
                elements. We carefully remove deteriorated mortar and replace it
                to restore strength and weather resistance.
              </p>
              <ul className="space-y-3 text-sm font-bold uppercase tracking-widest text-[#F9D759]">
                <li className="flex items-center gap-2">
                  <CheckCircle2 size={16} /> Raking out sandy mortar
                </li>
                <li className="flex items-center gap-2">
                  <CheckCircle2 size={16} /> High-durability replacement
                </li>
              </ul>
            </div>

            <div className="group p-10 bg-slate-100 text-[#243453] rounded-[3rem] border-b-8 border-[#F9D759]">
              <Droplets className="text-[#243453] mb-6" size={48} />
              <h3 className="text-2xl font-black uppercase mb-4">
                Lead Flashing
              </h3>
              <p className="text-slate-600 text-lg leading-relaxed mb-6 font-medium">
                Leadwork is essential for a watertight seal. We repair or
                replace chimney leadwork with careful attention to detail,
                ensuring it performs reliably over time.
              </p>
              <ul className="space-y-3 text-sm font-bold uppercase tracking-widest text-slate-400">
                <li className="flex items-center gap-2">
                  <CheckCircle2 className="text-[#F9D759]" size={16} />{" "}
                  Precision Code 4 Lead
                </li>
                <li className="flex items-center gap-2">
                  <CheckCircle2 className="text-[#F9D759]" size={16} /> Secure
                  Watertight Seal
                </li>
              </ul>
            </div>
          </div>
        </div>
      </section>

{/* 5. THE "PHOTO EVIDENCE" PROMISE */}
      <section className="py-24 bg-white px-4 md:px-10">
        <div className="max-w-screen-2xl mx-auto">
          <div className="bg-[#F9D759] border-6 border-[#243453] rounded-[2rem] lg:rounded-[3rem] overflow-hidden flex flex-col lg:flex-row shadow-2xl items-stretch">
            
            <div className="lg:w-1/2 p-8 md:p-12 text-[#243453]">
              <h2 className="text-2xl md:text-4xl font-black uppercase mb-8 leading-tight tracking-tight text-[#243453]">
                Transparency <br />
                <span className="opacity-70 italic font-medium">From the Rooftop.</span>
              </h2>

              <div className="space-y-8">
                {[
                  {
                    icon: Camera,
                    title: "Photo Inspection",
                    desc: "Since chimneys are difficult to assess from the ground, we show you exactly what we find.",
                  },
                  {
                    icon: FileText,
                    title: "Clear Quotations",
                    desc: "Fixed-price quotes based on high-res photographic evidence. No guessing.",
                  },
                  {
                    icon: ShieldCheck,
                    title: "Reliable Repairs",
                    desc: "Focusing on identification and resolution of the actual cause, not short-term fixes.",
                  },
                ].map((promise, idx) => (
                  <div key={idx} className="flex items-center gap-5 group">
                    <div className="bg-[#243453] p-2.5 rounded-xl shrink-0 shadow-md">
                      <promise.icon className="text-[#F9D759]" size={24} />
                    </div>
                    <div>
                      <h4 className="text-lg font-black uppercase mb-1 tracking-wider">
                        {promise.title}
                      </h4>
                      <p className="text-[#243453]/80 text-sm md:text-base leading-relaxed font-bold">
                        {promise.desc}
                      </p>
                    </div>
                  </div>
                ))}
              </div>
            </div>

            <div className="lg:w-1/2 w-full self-stretch overflow-hidden">
              <div 
                className="h-full min-h-[400px] bg-[url('/images/chimney-repairs-2.jpg')] bg-cover bg-center border-t-6 lg:border-t-0 lg:border-l-6 border-[#243453]"
                aria-label="Action Roofing Chimney Repair Proof"
              />
            </div>
          </div>
        </div>
      </section>

      <ServicesCTA />

      {/* 6. FINAL WORD (Brother's Closing Statement) */}
      <footer className="py-24 bg-white text-center">
        <div className="max-w-screen-2xl mx-auto px-4 md:px-10">
          <div className="max-w-3xl mx-auto">
            <h3 className="text-xl font-black uppercase text-[#243453] mb-6 tracking-tight">
              Clear Advice. Reliable Workmanship.
            </h3>
            <p className="text-lg text-slate-600 italic leading-relaxed mb-10">
              "Chimney problems can be difficult to assess from the ground,
              which is why we focus on clear explanations and honest
              recommendations. We prioritize doing the job properly and ensuring
              your home is protected."
            </p>
            <div className="flex flex-col items-center">
              <div className="h-1.5 w-16 bg-[#F9D759] mb-6"></div>
              <p className="font-black text-[#243453] uppercase tracking-widest text-sm">
                Action Roofing Services | Chimney Specialists
              </p>
            </div>
          </div>
        </div>
      </footer>
    </div>
  );
};

export default ChimneyRepairsPage;
